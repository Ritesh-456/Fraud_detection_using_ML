...
Original file is located at
    https://colab.research.google.com/drive/1afGHmAuu_mpqJnRRB4a0GS-DDgrYHW-D
"""
...
class HybridFraudDetector:
    ...
    def _extract_link_features(self, url):
        try:
            ...
            domain_score = sum(
                1 for domain in suspicious_domains if domain in url.lower())
    ...
    def analyze_url(self, url):
        try:
            if not self.link_model or not self.link_scaler:
                raise ValueError(
                    "Model not trained. Please run train_model() first.")
    ...
    if detector.train_model():
        ...
        for url in test_urls:
            ...
            if result:
                print(f"\nURL: {url}")
                print(
                    f"Fraud Detection: {'⚠️ Fraudulent' if result['is_fraud'] else '✅ Legitimate'}")
